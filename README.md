...
```
# 7. Consumindo uma API para listar tarefas
Agora que já temos toda a estrutura da aplicação funcional, vamos adicionar uma integração com a API de tarefas.
Para isso, vamos consumir o endpoint de listar todas as tarefas em:
GET /api/todo

# Definindo a entidade TodoEntity
```typescript
// app/lib/todo.entity.ts
...
// configurando useEffect para processamento ao carregar componente
useEffect(() => {
...
// funcoes para criacao do componente card
...
// funcao para renderizar a lista de cards
function createCards() {
    return todoList?.map(t => createCard(t));
}

// nova estrutura da pagina todo
<AppContainer title="TODO IT" subtitle="" headline="">
    <div className="flex flex-col justify-center align-middle items-center space-y-5">
        { createCards() }
        <div className="w-full content-end text-right p-5">
